...
namespace UltraPaste.Utilities
{
    internal static class TrackEventHelper
    {
        ...
        {
            return vegas.GenerateEvents<T>(vegas.Project, path, start, length, useMultipleSelectedTracks, newTrackIndex);
        }

        // a complex implementation to import File as Events to Timeline
        // when media path is invalid, it won't generate any event
        // this method enables you to explicitly specify the project, rather than just the current one
        public static List<T> GenerateEvents<T>(this Vegas vegas, Project project, string path, Timecode start, Timecode length = null, bool useMultipleSelectedTracks = false, int newTrackIndex = -1) where T : TrackEvent
        {
            ...
            return project.GenerateEvents<T>(media, start, length, useMultipleSelectedTracks, newTrackIndex);
        }
