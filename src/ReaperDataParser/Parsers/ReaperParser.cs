...
namespace ReaperDataParser.Parsers
{
    using Models;
    public static class ReaperParser
    {
        private static readonly string[] EnvelopeStrings = new[] { "ENVSEG", "VOLENV", "VOLENV2", "PANENV", "PANENV2", "MUTEENV", "TEMPOENVEX" };
        ...
        private static T ParseFromBlock<T>(ReaperBlock block) where T : class
        {
            ...
            if (typeof(T) == typeof(ReaperTempoEnvelope))
            {
                return ParseTempoEnvelopeBlock(block) as T;
            }
        ...
        private static ReaperData ParseDataBlock(ReaperBlock block)
        {
            ...
            foreach (ReaperBlock child in block.Children)
            {
                ...
                ReaperTempoEnvelope tempoEnv = ParseTempoEnvelopeBlock(child);
                if (tempoEnv != null)
                {
                    data.TempoEnvelope = tempoEnv;
                    continue;
                }
        ...
        private static ReaperSource ParseSourceBlock(ReaperBlock block)
        {
            ...
            foreach (string[] tokens in TokenizeLines(block.Lines))
            {
                switch (tokens[0].ToUpperInvariant())
                {
                    case "<SOURCE":
                        ...
                        {
                            sourceType = tokens[1];
                            if (tokens[1].Equals("SECTION", StringComparison.OrdinalIgnoreCase))
                            {
                                source = new ReaperSourceSection();
                            }
                            else if (tokens[1].Equals("MIDI", StringComparison.OrdinalIgnoreCase))
                            {
                                source = new ReaperSourceMidi();
                            }
                        }
            ...
            else if (source is ReaperSourceMidi midi)
            {
                StringBuilder midiDataBuilder = new StringBuilder();
                ...
                foreach (string[] tokens in TokenizeLines(block.Lines))
                {
                    ...
                    {
                        case "HASDATA":
                            EnsureTokenCount(tokens, 2, "MIDI HASDATA");
                            midi.HasData = ParseBoolToken(tokens[1]);
                            if (tokens.Length > 2)
                            {
                                midi.PPQ = ParseIntToken(tokens[2]);
                            }
                            if (tokens.Length > 3)
                            {
                                midi.TimeBase = tokens[3];
                            }
                            break;
                        case "CCINTERP":
                            EnsureTokenCount(tokens, 2, "MIDI CCINTERP");
                            midi.CCInterp = ParseIntToken(tokens[1]);
                            break;
                        case "POOLEDEVTS":
                            if (tokens.Length > 1)
                            {
                                midi.PooledEvents = tokens[1];
                            }
                            break;
                        case "IGNTEMPO":
                            if (tokens.Length > 1)
                            {
                                midi.IgnoreTempo = string.Join(" ", tokens, 1, tokens.Length - 1);
                            }
                            break;
                        case "SRCCOLOR":
                            EnsureTokenCount(tokens, 2, "MIDI SRCCOLOR");
                            midi.SourceColor = ParseIntToken(tokens[1]);
                            break;
                        case "EVTFILTER":
                            if (tokens.Length > 1)
                            {
                                midi.EventFilter = string.Join(" ", tokens, 1, tokens.Length - 1);
                            }
                            break;
                        case "VELLANE":
                            if (tokens.Length > 1)
                            {
                                midi.VelLane = string.Join(" ", tokens, 1, tokens.Length - 1);
                            }
                            break;
                        case "CFGEDITVIEW":
                            if (tokens.Length > 1)
                            {
                                midi.ConfigEditView = string.Join(" ", tokens, 1, tokens.Length - 1);
                            }
                            break;
                        case "KEYSNAP":
                            EnsureTokenCount(tokens, 2, "MIDI KEYSNAP");
                            midi.KeySnap = ParseIntToken(tokens[1]);
                            break;
                        case "TRACKSEL":
                            EnsureTokenCount(tokens, 2, "MIDI TRACKSEL");
                            midi.TrackSel = ParseIntToken(tokens[1]);
                            break;
                        case "CFGEDIT":
                            if (tokens.Length > 1)
                            {
                                midi.ConfigEdit = string.Join(" ", tokens, 1, tokens.Length - 1);
                            }
                            break;
                        case "E":
                        case "X":
                            if (midiDataBuilder.Length > 0)
                            {
                                midiDataBuilder.AppendLine();
                            }
                            midiDataBuilder.Append(string.Join(" ", tokens));
                            break;
                        default:
                            break;
                    }
                ...
                midi.MIDIData = midiDataBuilder.ToString();
            }
        ...
        private static ReaperTempoEnvelope ParseTempoEnvelopeBlock(ReaperBlock block)
        {
            if (block?.Type != "TEMPOENVEX")
            {
                return null;
            }

            ReaperTempoEnvelope env = new ReaperTempoEnvelope();
            ...
            foreach (string[] tokens in TokenizeLines(block.Lines))
            {
                switch (tokens[0].ToUpperInvariant())
                {
                    ...
                    case "ACT":
                        ...
                        break;
                    case "VIS":
                        env.Vis = ParseIntArray(tokens);
                        break;
                    case "LANEHEIGHT":
                        env.LaneHeight = ParseIntArray(tokens);
                        break;
                    case "ARM":
                        EnsureTokenCount(tokens, 2, "tempo envelope ARM");
                        env.Arm = ParseIntToken(tokens[1]);
                        break;
                    case "DEFSHAPE":
                        env.DefShape = ParseIntArray(tokens);
                        break;
                    case "PT":
                        env.Points.Add(ParseDoubleArray(tokens));
                        break;
